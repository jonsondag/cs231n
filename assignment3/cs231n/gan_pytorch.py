...
def discriminator_loss(logits_real, logits_fake):
    ...
    # targets = torch.cat([torch.ones(logits_real.size()).type(dtype), torch.zeros(logits_fake.size()).type(dtype)])
    # loss = bce_loss(logits, targets)
    real_loss = bce_loss(logits_real, torch.ones(logits_real.size()).type(dtype))
    fake_loss = bce_loss(logits_fake, torch.zeros(logits_fake.size()).type(dtype))
...
def get_optimizer(model):
    ...
    optimizer = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, betas=(0.6, 0.999))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return optimizer
